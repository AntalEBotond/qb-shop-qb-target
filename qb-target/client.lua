...
local function LeftTarget()
	-- Do not drop NUI focus here so the cursor remains available
	success = false
	table_wipe(sendData)
...
local function EnableTarget()
	...
	-- Always enable NUI focus so the mouse cursor is used for interaction
	SetCursorLocation(0.5, 0.5)
	SetNuiFocus(true, true)
	SetNuiFocusKeepInput(true)
	hasFocus = true
...
CreateThread(function()
	if Config.Toggle then
        ...
        if not Config.Toggle then
                -- Additional cursor helper when using hold-to-target mode
                RegisterCommand('toggleTargetCursor', function()
                        -- If NUI already focused, close everything
                        if hasFocus then
                                DisableTarget(true)
                                return
                        end

                        -- Ensure target eye is active first
                        if not targetActive then
                                CreateThread(EnableTarget)
                                -- Wait until the target loop is active
                                local timeout = 0
                                while not targetActive and timeout < 200 do
                                        Wait(0)
                                        timeout = timeout + 1
                                end
                        end

                        -- Open the NUI with whatever options are currently available
                        EnableNUI(nuiData)
                end, false)

                RegisterKeyMapping('toggleTargetCursor', ('Toggle target cursor (%s)'):format(Config.OpenKey), 'keyboard', Config.OpenKey)
                TriggerEvent('chat:removeSuggestion', '/toggleTargetCursor')
        end
